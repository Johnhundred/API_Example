  ...
  Object.keys(obj).forEach((key) => {
    ...
    if (helpers.isArray(obj[key])) {
      ...
      if (helpers.isObject(obj[key][0])) {
        obj[key] = helpers.getXssFilteredArrayOfObjects(obj[key]); // eslint-disable-line
      } else {
        obj[key] = helpers.getXssFilteredArray(obj[key]); // eslint-disable-line
      }
    } else if (helpers.isObject(obj[key])) {
      // Current object value was an object, trigger recursion to iterate over the nested object
      obj[key] = helpers.getXssFilteredObject(obj[key]); // eslint-disable-line
    }

    // Current object value was not an array or object, is it a string?
    if (helpers.isString(obj[key])) {
      obj[key] = helpers.getXssFilteredVar(obj[key]); // eslint-disable-line
    }
  });
